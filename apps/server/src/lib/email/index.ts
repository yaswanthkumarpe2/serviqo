 ...
 * the failure this guards against. So production without a real provider is
 * a startup error, not a runtime surprise discovered at the first
 * registration.
 *
 ...
 * implementation exists, and a selector with a single arm is a switch
 * waiting for a second vendor. Both arrive together or not at all.
 */
export function resolveEmailProvider(): EmailProvider {
  if (env.NODE_ENV === "production") {
    throw new Error(
      "No production EmailProvider is configured. ConsoleEmailProvider logs instead of delivering and must never serve production traffic.",
    );
  }
